...
static void ssw_write (s_align* a,
			...
			int8_t scoreOnly) {	//0: No effect; 1: Only output the alignment score value
	if(scoreOnly){
		fprintf(stdout, "%d, ", a->score1); //Only print the optimal alignment score
	}
	else if (sam == 0) {	// Blast like output
...
step3:
		...
		if (a->score1 == 0) fprintf(stdout, "4\t*\t0\t255\t*\t*\t0\t0\t*\t*\n");
			...
			int32_t mismatch = mark_mismatch(a->ref_begin1, a->read_begin1, a->read_end1, ref_num, read_num, read->seq.l, &a->cigar, &a->cigarLen);
...
int main (int argc, char * const argv[]) {
	...
	int32_t l, m, k, match = 2, mismatch = 2, gap_open = 3,
		gap_extension = 1, path = 0, reverse = 0, n = 5, sam = 0, scoreOnly = 0,
		protein = 0, header = 0, s1 = 67108864, s2 = 128, filter = 0;
	...
	// Parse command line.
	while ((l = getopt(argc, argv, "m:x:o:e:a:f:pcrshl")) >= 0) {
		switch (l) {
	...
	if (optind + 2 > argc) {
		...
		fprintf(stderr, "\t-l\tOnly output the score. Not compatible with -c/-s\n\n");
		return 1;
	}
	if(scoreOnly && (sam || header || path)){
		fprintf(stderr, "Cannot use -l (score only) with -s or -c or -h.\n\n");
		return 1;
	}
	...
	while (kseq_read(read_seq) >= 0) {
		...
		while (kseq_read(ref_seq) >= 0) {
			...
			if (result_rc && result_rc->score1 > result->score1 && result_rc->score1 >= filter) {
				if (sam) ssw_write (result_rc, ref_seq, read_seq, read_rc, ref_num, num_rc, table, 1, 1, scoreOnly);
				else ssw_write (result_rc, ref_seq, read_seq, read_rc, ref_num, num_rc, table, 1, 0, scoreOnly);
			}else if (result && result->score1 >= filter){
				if (sam) ssw_write(result, ref_seq, read_seq, read_seq->seq.s, ref_num, num, table, 0, 1, scoreOnly);
				else ssw_write(result, ref_seq, read_seq, read_seq->seq.s, ref_num, num, table, 0, 0, scoreOnly);
